...
/**
 * Helper function to retrieve a header pointer from a pointer and an 
 *        offset
 */
static inline header * get_header_from_offset(void * ptr, ptrdiff_t off) {
...
/**
 * Helper function to get the header to the right of a given header
 */
...
/**
 * Helper function to get the header to the left of a given header
 */
...
/**
 * Helper to verify that there are no unlinked previous or next pointers
 *        in the free list
 */
static inline header * verify_pointers() {
  for (int i = 0; i < N_LISTS; i++) {
...
/**
 * Verify the structure of the free list is correct by checkin for 
